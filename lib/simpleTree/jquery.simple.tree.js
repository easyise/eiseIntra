$.fn.simpleTree = function(opt){
	return this.each(function(){
		...
		TREE.option = {
			...
			, afterNodeToggle: false // hack by IE
		};
		...
		TREE.nodeToggle = function(obj)
		{
			...
			// hack by IE 2016-05-13
			var _afterNodeToggle = function(){
				if(typeof TREE.option.afterNodeToggle ==='function'){
					TREE.option.afterNodeToggle.call(childUl);
				}	
			}
			...
			if(childUl.is(':visible')){
				...
				if(TREE.option.animate)
				{
					childUl.animate({height:"toggle"},TREE.option.speed, function(){_afterNodeToggle();});
				}else{
					childUl.hide();
					_afterNodeToggle();
				}
        ...
        TREE.initFilePath = function(){ // ADDED BY IEL 11.02.2013
            ...
            TREE.filePath = (arrPath[0]=='' ? "/" : '') + strFilePath+"/";
        }
        TREE.initFilePath();
		TREE.init(ROOT);
